...
const VIDEO_PROJECTS_CSS_FACADE_IMPORT = "./styles/features/video-projects/index.css";
const VIDEO_PROJECTS_FACTORY_EXPORT = 'createVideoProjectsFeature';
const VIDEO_PROJECTS_RENDER_FACADE_EXPORT = 'renderSelectedVideoProjectView';
...
function assertVideoProjectsFacadeBoundary({ stylesSource = '', videoProjectsFacadeSource = '' }, failures) {
  if (stylesSource && !stylesSource.includes(VIDEO_PROJECTS_CSS_FACADE_IMPORT)) {
    failures.push(`styles.css must import ${VIDEO_PROJECTS_CSS_FACADE_IMPORT}`);
  }
  if (videoProjectsFacadeSource && !videoProjectsFacadeSource.includes(VIDEO_PROJECTS_FACTORY_EXPORT)) {
    failures.push(`video-projects facade must export ${VIDEO_PROJECTS_FACTORY_EXPORT}`);
  }
  if (videoProjectsFacadeSource && !videoProjectsFacadeSource.includes(VIDEO_PROJECTS_RENDER_FACADE_EXPORT)) {
    failures.push(`video-projects facade must preserve ${VIDEO_PROJECTS_RENDER_FACADE_EXPORT} render contract`);
  }
}

export function runParityChecklist({
  indexHtmlSource,
  mainJsSource,
  compositionRootSource,
  appShellSource,
  stylesSource,
  videoProjectsFacadeSource,
}) {
  const failures = [];
  ...
  assertVideoProjectsFacadeBoundary({ stylesSource, videoProjectsFacadeSource }, failures);
