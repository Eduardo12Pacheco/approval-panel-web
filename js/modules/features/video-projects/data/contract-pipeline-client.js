...
export function normalizePreparedContractRows(rows = []) {
  ...
  return rows.map((row, index) => {
    ...
    return {
      ...
      motionPresetId: row?.motionPresetId || (typeof row?.motion === 'string' ? row.motion : 'Zoom 125'),
      motion: row?.motion || (mediaMode === 'newspaper' ? { fromScale: 1, toScale: 1.25, fromX: 0, fromY: 0, toX: 0, toY: 0, easing: 'linear' } : 'Zoom 125'),
